...
export default function AlertModal({ isOpen, onClose, title, message, type = 'info' }: AlertModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[120] backdrop-blur-sm flex items-center justify-center px-4" style={{ background: 'rgba(0, 0, 0, 0.6)' }}>
            <Card className="w-full max-w-sm p-0 rounded-[2rem] overflow-hidden shadow-2xl relative animate-in fade-in zoom-in duration-200" style={{ background: 'var(--background)', borderColor: 'var(--border-color)' }} onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col items-center text-center p-8">
                    {getIcon()}
                    <h3 className="text-2xl font-black mb-2">{title || defaultTitle}</h3>
