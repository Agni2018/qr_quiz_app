...
export default function BadgeChallengeHeader({ type, activeTab }: BadgeChallengeHeaderProps) {
    ...
    return (
        <div 
            className="flex flex-col md:flex-row items-start md:items-center justify-between animate-fade-in w-full gap-6 md:gap-8"
            style={{ marginTop: '20px', marginBottom: '40px' }}
        >
            {/* Main Header */}
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6 group">
                <div className="flex flex-col gap-1">
                    <h1 className="text-3xl md:text-5xl font-black tracking-tighter text-slate-900" style={{color:'black'}}>
            ...
            {/* Navigation Tabs */}
            <div 
                className="flex flex-col sm:flex-row items-center p-2 bg-white backdrop-blur-xl border border-slate-200 rounded-[1.5rem] sm:rounded-full w-full sm:w-fit gap-2 shadow-2xl overflow-hidden"
                style={{ marginTop: '20px', padding: 12 }}
            >
                <Link href={badgesPath} className="w-full sm:w-auto">
                    <Button
                        variant={activeTab === 'badges' ? 'primary' : 'ghost'}
                        className={`h-11 md:h-12 px-8 rounded-xl sm:rounded-full font-bold transition-all flex items-center justify-center gap-3 w-full sm:w-auto border-none ${
                            activeTab === 'badges' 
                            ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20' 
                            : 'text-slate-500 hover:text-black hover:bg-slate-50 bg-transparent'
                        }`}
                        style={activeTab === 'badges' ? { background: '#4f46e5', color: 'white' } : {}}
                    >
                ...
                <Link href={challengesPath} className="w-full sm:w-auto">
                    <Button
                        ...
                        style={activeTab === 'challenges' ? { background: '#4f46e5', color: 'white' } : {}}
                    >
